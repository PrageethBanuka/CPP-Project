...
Simulation::Simulation(const Config& config)
    ...
    this->numThreads = config.initial_threads;
...
void Simulation::handleCollisions() {
    for (size_t i = 0; i < particles.size(); ++i) {
        for (size_t j = i + 1; j < particles.size(); ++j) {
            ...
            double collisionDistance = particles[i]->getRadius() + particles[j]->getRadius();
            
            if (distance < collisionDistance) {
                double tempVX = particles[i]->getVX();
                double tempVY = particles[i]->getVY();
                
                particles[i]->setVelocity(particles[j]->getVX(), particles[j]->getVY());
                particles[j]->setVelocity(tempVX, tempVY);
